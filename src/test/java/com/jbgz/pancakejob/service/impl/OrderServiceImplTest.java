    ...
    void whiteBoxTest1() {
        ...
        try {
            assertDoesNotThrow(()->{
                ...
                AppealDealVO appealDealVO = new AppealDealVO();
                appealDealVO.setOrderId(orderId);
                appealDealVO.setAppealType("求职者评价申诉");
                appealDealVO.setStatus(true);
                appealDealVO.setAppealResult("评分有失偏颇");
                res = appealService.saveDealResult(appealDealVO);
                assertTrue(res);
                appealDealVO.setAppealType("招聘方评价申诉");
                appealDealVO.setStatus(false);
                appealDealVO.setAppealResult("评分很合理");
                res = appealService.saveDealResult(appealDealVO);
                assertTrue(res);
            });
        } catch (Exception e) {
